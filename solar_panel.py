...
blockchain.register_device("smart_solar_panel")  # Register this device
...
def on_connect(client, userdata, flags, rc):
    print("Smart Solar Panel connected to broker.")
...
def request_chain_sync(client):
    ...
    client.publish(topic_sync_request, json.dumps({"requester": "smart_solar_panel"}))
...
def main():
    client = mqtt.Client("SmartSolarPanelNode")
    ...
    try:
        ...
        while True:
            ...
            # Attempt to propose a block every 15 seconds
            if current_time - last_publish_time >= 15:
                watts = randint(100, 500)  # Simulate power generation in watts
                try:
                    propose_block(client, {"device": "smart_solar_panel", "data": {"watts": watts}})
                except:
        ...
        print("Exiting Smart Solar Panel...")
    finally:
        client.loop_stop()
        client.disconnect()
