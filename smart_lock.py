...
def on_connect(client, userdata, flags, rc):
    print("Smart Lock connected to broker with result code", rc)
...
def handle_sync_request(client):
    try:
        ...
        print("Smart Lock published full blockchain in response to sync request.")
    except Exception as e:
...
def request_chain_sync(client, max_sync_attempts):
    try:
        ...
        client.publish(topic_sync_request, json.dumps({"requester": "Smart Lock", "attempts": max_sync_attempts}))
    except Exception as e:
...
def publish_new_block(client, data):
    try:
        ...
        print("Smart Lock published new block:", block_data)
    except Exception as e:
...
def main():
    ...
    try:
        while True:
            lock_status = {"status": "locked" if int(time.time()) % 2 == 0 else "unlocked"}
            publish_new_block(client, lock_status)
            time.sleep(10)
